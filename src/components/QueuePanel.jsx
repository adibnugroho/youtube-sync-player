...
const QueuePanel = ({ queue, currentVideoId, onAddVideo, onRemoveVideo, onSkipVideo, onReorderQueue, onPlayNow }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!videoId) {
      setError('Invalid YouTube link. Use youtube.com/watch?v=... or youtu.be/... format.');
      return;
    }
    ...
    try {
      ...
      console.warn("Failed to fetch YouTube title (oEmbed blocked or rate limited)", err);
    }
  ...
  const handlePlayNext = (index) => {
    ...
    newQueue.splice(1, 0, item);
    if (onReorderQueue) onReorderQueue(newQueue);
  };

  const handlePlayNowLocal = (id, index) => {
    if (index === 0 || !onPlayNow) return;
    onPlayNow(id, index);
  };
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-yt-card rounded-2xl border border-yt-border overflow-hidden transition-colors duration-300">
      ...
      <div className="p-4 border-b border-yt-border bg-black/5 dark:bg-white/5 flex items-center gap-3">
        ...
        <h2 className="text-lg font-semibold tracking-wide text-yt-text flex-1">Video Queue</h2>
            ...
            return (
              ...
              >
                ...
                <div className="w-24 sm:w-28 aspect-video bg-black rounded-lg overflow-hidden shrink-0 relative">
                  <img src={`https://img.youtube.com/vi/${item.videoId}/mqdefault.jpg`} alt="Thumbnail" className="w-full h-full object-cover opacity-80 pointer-events-none" />
                  {isPlaying && (
                    <div className="absolute inset-0 bg-youtube-red/20 flex items-center justify-center">
                      <span className="text-[9px] sm:text-[10px] uppercase tracking-wider font-bold bg-youtube-red text-white px-2 py-0.5 rounded-sm shadow-lg pointer-events-none">Playing</span>
                    </div>
                  )}
                  {!isPlaying && (
                ...
                <div className="flex flex-col justify-center flex-1 min-w-0 pr-1">
                  <p className="text-sm font-medium line-clamp-2 text-yt-text" title={item.title || item.videoId}>{item.title || `Video ID: ${item.videoId}`}</p>
                  <p className="text-xs text-yt-muted mt-1 flex items-center gap-1.5">
                    <span className="w-4 h-4 rounded-full bg-black/10 dark:bg-white/10 flex items-center justify-center pt-px text-[9px] uppercase font-bold text-yt-text">{item.sender.charAt(0)}</span>
                    {item.sender}
                  </p>
                </div>
                ...
                {isDraggable && (
                   <div className="hidden group-hover:flex flex-col items-center justify-center gap-1 px-1 animate-in fade-in duration-200">
                     <button onClick={(e) => { e.stopPropagation(); handleMoveUp(index); }} disabled={index === 1} className="p-1 hover:bg-black/10 dark:hover:bg-white/10 rounded disabled:opacity-30 disabled:hover:bg-transparent text-yt-muted transition-colors" title="Move Up"><ArrowUp className="w-3.5 h-3.5" /></button>
                     <button onClick={(e) => { e.stopPropagation(); handleMoveDown(index); }} disabled={index === queue.length - 1} className="p-1 hover:bg-black/10 dark:hover:bg-white/10 rounded disabled:opacity-30 disabled:hover:bg-transparent text-yt-muted transition-colors" title="Move Down"><ArrowDown className="w-3.5 h-3.5" /></button>
                   </div>
                )}
                ...
                <div className="hidden group-hover:flex flex-col justify-center items-center gap-1 border-l border-yt-border/50 pl-2 animate-in fade-in duration-200">
                  {isDraggable && (
                     <>
                        <button onClick={(e) => { e.stopPropagation(); handlePlayNowLocal(item.id, index); }} className="p-1 px-2 w-full bg-youtube-red hover:bg-red-600 text-white rounded transition-colors flex items-center justify-center gap-1 shadow-sm" title="Play Now (Replaces current)">
                          <Play className="w-3 h-3" fill="currentColor" />
                          <span className="text-[9px] uppercase font-bold">Play</span>
                        </button>
                       {index > 1 && (
                         <button onClick={(e) => { e.stopPropagation(); handlePlayNext(index); }} className="p-1 px-2 w-full bg-youtube-red/10 hover:bg-youtube-red text-youtube-red hover:text-white rounded transition-colors flex items-center justify-center gap-1" title="Play Next (Move to #1)">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); onRemoveVideo(item.id); }} className="p-1.5 mt-1 hover:bg-red-500/10 rounded-lg text-yt-muted hover:text-red-500 transition-colors w-full flex justify-center" title="Remove from queue"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            );
          })
